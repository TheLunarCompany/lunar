...
test.describe('Dashboard Canvas Zero State', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForSelector('[class*="bg-gray-100"]', { timeout: LONG_TIMEOUT });
  });

  test('should display zero state when no servers are connected', async ({ page }) => {
    const canvasCard = page.locator('div[class*="flex-1"][class*="shadow-sm"]').first();
    await expect(canvasCard).toBeVisible();

    await page.waitForTimeout(WAIT_DELAY);

    const reactFlowCanvas = page.locator('.react-flow');
    const isReactFlowVisible = await reactFlowCanvas.isVisible().catch(() => false);
    ...
    if (isReactFlowVisible) {
      ...
      await expect(mcpxNode).toBeVisible({ timeout: TIMEOUT });
      await expect(noServersNode).toBeVisible({ timeout: TIMEOUT });
      await expect(noAgentsNode).toBeVisible({ timeout: TIMEOUT });
    } else {
  ...
  test('should display zero state card with correct text and icon', async ({ page }) => {
    await page.waitForTimeout(WAIT_DELAY);
    ...
    await expect(noServersCard).toBeVisible({ timeout: TIMEOUT });
  ...
  test('should display placeholder nodes in zero state', async ({ page }) => {
    ...
    await expect(reactFlow).toBeVisible({ timeout: TIMEOUT });
  ...
  test('should have "Add Server" button in zero state', async ({ page }) => {
    ...
    await expect(topPanelAddServerButton).toBeVisible({ timeout: TIMEOUT });
  ...
  test('should display "System Connectivity" title', async ({ page }) => {
    ...
    await expect(systemConnectivityTitle).toBeVisible({ timeout: TIMEOUT });
  });
});
