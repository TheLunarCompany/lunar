...
export const EditableBadge: React.FC<EditableBadgeProps> = ({
  ...
  return (
    <div className="flex items-center gap-2">
      {label && <div className="text-xl font-semibold px-4 py-2  text-gray-900">{label}</div>}
      ...
      {isEditing ? (
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-lg text-lg font-medium border bg-white text-gray-900 border-gray-300">
          ...
          />
          <Button variant="ghost" size="sm" onClick={handleConfirm} className="p-1 h-6 w-6">
            <Check className="w-3 h-3 text-green-600" />
          </Button>
          <Button variant="ghost" size="sm" onClick={handleCancel} className="p-1 h-6 w-6">
      ...
      {error && (
        <div id="editable-badge-error" className="text-xs text-red-600 px-2 py-1 bg-red-50 border border-red-200 rounded">
          {error}
        </div>
      )}
    </div>
