...
interface ToolDetailsDialogProps {
  ...
  tool: {
    ...
    overrideParams?: Record<string, { value?: string; description?: { text: string } }>;
  };
  providers?: Array<{ name: string; originalTools: Array<{ name: string; inputSchema?: any }> }>;
  onEdit?: () => void;
  onDuplicate?: () => void;
  onDelete?: () => void;
  onCustomize?: () => void;
  ...
  const parameterEntries = useMemo(() => {
    ...
    if (Array.isArray(tool.parameters)) {
      tool.parameters.forEach((param) => {
        ...
        const next = { ...existing } as { value?: string; description?: string };
    ...
    return Object.entries(tool.inputSchema.properties).map(
      ([paramName, paramSchema]: [string, any]) => ({
        ...
        providerOverride: providerSelectors.get(tool.serviceName || "")?.originalTools?.find(
          (t: any) => t.name === (tool.originalToolName || tool.name),
        )?.overrideParams?.[paramName]?.value,
      }),
    );
  }, [tool.inputSchema, tool.parameters, tool.overrideParams, providerSelectors, tool.serviceName, tool.originalToolName, tool.name]);
  ...
  return (
    <Sheet open={isOpen} onOpenChange={(open: boolean) => !open && onClose()}>
      ...
      >
                 ...
                 <div className={`flex ${tool.isCustom ? "justify-between" : "justify-end"}  items-center gap-2 border-b border-gray-200 px-6 py-4`}>
               {tool.isCustom && <CustomBadge
                      color="blue"
                      size="md"
                      rounded="lg"
          ...
          <div className="mb-3">
            <div className="flex items-center justify-between">
              ...
              <SheetTitle>
                ...
                <McpIcon style={{ color: providerSelectors.get(tool.serviceName).icon }} className="w-12 h-12" />
              )}

                <div className="flex flex-col">
              { tool.serviceName.charAt(0).toUpperCase() + tool.serviceName.slice(1)}  

                {tool.isCustom ?  <HierarchyBadge
...
/>}
        ...
        <div className="flex flex-col items-start justify-start">
          {tool.isCustom && (
        <div className="mb-6">
            <h3 className="text-base font-medium font-semibold  mb-1">Custom Tool Name</h3>
            <p className="text-[#231A4D] text-sm leading-relaxed">{tool.name}</p>
          </div>)}

        <div className="mb-6">
            <h3 className="text-base font-medium font-semibold  mb-1">Description</h3>
            <p className="text-[#231A4D] text-sm leading-relaxed">{tool.description}</p>
          </div>
          </div>
          ...
          {tool.inputSchema?.properties &&
            Object.keys(tool.inputSchema.properties).length > 0 && (
              <div >
                <h3 className="text-base font-medium text-gray-800 mb-1">Parameters</h3>
             
                  {parameterEntries.length > 0 ? (
                    <div className="space-y-4  gap-4 pb-4">
                      {parameterEntries.map(({ name, schema, value, descriptionOverride }) => (
                        <div key={name} className="space-y-3  rounded-lg bg-[#F9F8FB]  border border-gray-200 rounded-lg p-3">
                          <div>
                            ...
                            {(descriptionOverride || schema.description) && (
                              <p className="text-xs text-[#827E95] leading-relaxed">
                                {tool?.overrideParams?.[name]?.description?.text || schema.description}
                              </p>
                            )}
                          </div>
                        </div>
