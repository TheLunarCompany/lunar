...
export default function AgentControls({
  ...
  const [internalAccessConfig, setInternalAccessConfig] = useState(
    agent?.access_config || [],
  );
  ...
  const getServerAccessConfig = (serverId) => {
    return (
      internalAccessConfig.find((conf) => conf.server_id === serverId) || {
        server_id: serverId,
        server_name:
          mcpServers.find((s) => s.id === serverId)?.name || "Unknown Server",
        allow_server: false,
        tools: [],
      }
    );
  ...
  const ToggleLabel = ({ enabled }) => (
    <span
      className={`text-[8px] font-medium flex items-center mr-1 ${enabled ? "text-[var(--color-fg-success)]" : "text-[var(--color-fg-danger)]"}`}
    >
      {enabled ? (
            ...
            {mcpServers.length > 0 ? (
              mcpServers.map((server) => {
                ...
                return (
                  <div
                    key={server.id}
                    className="p-1 border border-[var(--color-border-primary)] rounded bg-[var(--color-bg-container-overlay)]"
                  >
                            ...
                            return (
                              ...
                              >
                                <span className="text-[9px] text-[var(--color-text-secondary)] flex items-center gap-0.5">
                                  <SlidersHorizontal className="w-1.5 h-1.5" />{" "}
                                  {tool.name}
                                </span>
                                <div className="flex items-center">
                                  <ToggleLabel
                                    enabled={toolConfig.allow_tool}
                                  />
                                  <Switch
              ...
              <p className="text-[9px] text-center text-[var(--color-text-secondary)] py-1">
                No MCP Servers.
              </p>
            )}
          </div>
