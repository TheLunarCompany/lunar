...
const CustomCurvedEdge: React.FC<EdgeProps> = ({
  ...
  const connectionPointX =
    column !== undefined && column >= 0
      ? SERVER_NODE_INITIAL_GAP -
        110 +
        column * (NODE_WIDTH + ZERO_STATE_PADDING + COLUMN_CONNECTION_PADDING)
      : sourceX;
  ...
  const nodesInColumn =
    typeof data?.nodesInColumn === "number" ? data.nodesInColumn : undefined;
  const shouldConnectToCenterForMultipleNodes =
    nodesInColumn !== undefined && nodesInColumn > 1;
