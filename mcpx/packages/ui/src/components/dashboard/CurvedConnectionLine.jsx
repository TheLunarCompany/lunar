...
const CurvedConnectionLine = ({ startX, startY, endX, endY, active, lineId }) => {
  const arrowId = `arrow-${lineId}`;
  const lineClass = active ? 'stroke-[var(--color-fg-success)] pulsing-line' : 'stroke-[var(--color-text-disabled)]';
  ...
  return (
    <svg className="overflow-visible absolute pointer-events-none" style={{ left: 0, top: 0, width: '100%', height: '100%' }}>
      <defs>
        <marker
          id={arrowId}
          markerWidth="12"
        ...
        >
          <path d="M0,0 L5,3.5 L0,7 Z" className={active ? "fill-[var(--color-fg-success)]" : "fill-[var(--color-text-disabled)]"} />
        </marker>
      </defs>
      <path
        d={pathData}
