...
const generateSampleTokenData = (currentUsage) => {
  ...
  return [
    {
      name: "Jan",
      tokens: Math.max(0, baseTokens - Math.floor(Math.random() * 100)),
    },
    {
      name: "Feb",
      tokens: Math.max(0, baseTokens - Math.floor(Math.random() * 80)),
    },
    {
      name: "Mar",
      tokens: Math.max(0, baseTokens - Math.floor(Math.random() * 60)),
    },
    {
      name: "Apr",
      tokens: Math.max(0, baseTokens - Math.floor(Math.random() * 40)),
    },
    {
      name: "May",
      tokens: Math.max(0, baseTokens - Math.floor(Math.random() * 20)),
    },
    { name: "Jun", tokens: baseTokens },
  ];
};

export default function MCPXAnalytics({
  configurationData,
  mcpServers,
  aiAgents,
}) {
  ...
  const lastUpdated = configurationData.lastUpdatedAt
    ? new Date(configurationData.lastUpdatedAt)
    : new Date();
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        {analyticsTiles.map((tile) => {
          ...
          return (
            <Card
              key={tile.title}
              className={`bg-[var(--color-bg-container-overlay)] border-[var(--color-border-primary)] ${tile.comingSoon ? "opacity-50" : ""}`}
            >
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1 pt-2 px-2.5">
                <CardTitle className="text-[10px] font-medium text-[var(--color-text-secondary)]">
                  ...
                  {tile.comingSoon && (
                    <Badge
                      variant="outline"
                      className="ml-1 text-[6px] px-1 py-0"
                    >
                      Coming Soon
                    </Badge>
                  )}
                ...
                <Icon
                  className="h-3 w-3 text-[var(--color-text-disabled)]"
                  style={{ color: tile.color }}
                />
              </CardHeader>
              <CardContent className="pb-2 px-2.5">
                <div
                  className={`text-xs font-bold truncate ${tile.comingSoon ? "text-[var(--color-text-disabled)]" : "text-[var(--color-text-primary)]"}`}
                  style={{ color: tile.comingSoon ? undefined : tile.color }}
                >
