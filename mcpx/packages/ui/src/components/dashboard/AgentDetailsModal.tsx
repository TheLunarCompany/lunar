...
export const AgentDetailsModal = ({ agent, isOpen, onClose }: AgentDetailsModalProps) => {
  const [searchQuery, setSearchQuery] = useState("");
  const [expandedGroups, setExpandedGroups] = useState<Set<string>>(new Set());
  const [allowAll, setAllowAll] = useState(false);
  const [originalToolGroups, setOriginalToolGroups] = useState<Set<string>>(new Set());
  const [editedToolGroups, setEditedToolGroups] = useState<Set<string>>(new Set());
  const navigate = useNavigate();
  
  const { toolGroups, profiles, setProfiles, appConfigUpdates, hasPendingChanges, resetAppConfigUpdates } = useAccessControlsStore((s) => ({
  ...
  const hasChanges = useMemo(() => {
    ...
    const agentProfile = currentProfiles.find(profile => 
      profile && profile.name !== "default" && profile.agents && profile.agents.includes(agent.identifier)
    );
  ...
  const agentToolGroups = useMemo(() => {
    ...
    try {
      const agentProfile = profiles.find(profile => 
        profile?.name !== "default" && 
        profile?.agents?.includes(agent.identifier)
      );
      
      const createToolGroup = (toolGroup: any, enabled: boolean) => {
        const allTools = Object.values(toolGroup.services || {}).flat() as string[];
  ...
  useEffect(() => {
    if (agent && !isInitialized) {
      const agentProfile = profiles?.find(p => 
        p?.name !== "default" && p?.agents?.includes(agent.identifier)
      );
  ...
  const filteredGroups = agentToolGroups.filter(group =>
    group && group.title && group.title.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  const saveConfiguration = useCallback(async () => {
    ...
    let agentProfile = currentProfiles.find(profile => 
      profile && profile.name !== "default" && profile.agents && profile.agents.includes(agent.identifier)
    );
      ...
      <SheetContent side="right" className="!w-[600px] !max-w-[600px] bg-white p-0 flex flex-col [&>button]:hidden">
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-4">
            <div className="grid grid-cols-4 gap-4 text-sm">
              ...
              <div className="text-left">
                ...
                <div className="text-gray-800">
                  {agent.usage?.lastCalledAt ? formatDateTime(agent.usage.lastCalledAt) : "N/A"}
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="px-4 pt-4 pb-0 flex-1 flex flex-col overflow-hidden">
          <div className="flex items-center justify-between mb-4 flex-shrink-0">
            <h3 className="text-lg font-semibold text-gray-800">Select Tool Groups</h3>
          ...
          <div className="space-y-3 flex-1 overflow-y-auto pb-6 min-h-0">
            {filteredGroups.length === 0 ? (
              <div className="text-center py-8 bg-gray-50 rounded-lg border border-gray-200">
                <h4 className="font-semibold text-gray-800 mb-2">No Tool Groups Defined</h4>
                <p className="text-gray-600 mb-4">Create a Tool Group for effective agent control.</p>
                <p className="text-gray-500 text-sm mb-4">Go to the Tool Catalog area to set this up.</p>
                <Button 
              ...
              filteredGroups.map((group) => (
                <Card key={group.id} className="border border-gray-200 bg-white">
                  <CardHeader className="p-3 pb-2">
                    <div className="flex items-center justify-between">
                      ...
                      <Switch 
                        ...
                        disabled={allowAll}
                        onCheckedChange={(checked) => handleToolGroupToggle(group.id, checked)}
                      />
                    </div>
                  </CardHeader>
                  <CardContent className="p-3 pt-0">
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      {group.mcpNames.map((mcpName, index) => (
                        <Badge key={index} variant="outline" className="text-sm bg-white border-gray-200">
                          {mcpName}
                        </Badge>
                      ))}
                    ...
                    {expandedGroups.has(group.id) && (
                        ...
                        <div className="flex items-center mb-2 flex-wrap gap-2">
                            {group.allTools.map((tool, index) => (                                           
                              <Badge key={index} variant="outline" className="text-xs bg-gray-100 rounded-none">
                                {tool}
                              </Badge>
                            ))}
                          </div>
                    ...
                    <div className="flex justify-start">
                      ...
                      >
                        {expandedGroups.has(group.id) ? (
                          <>View Less <ChevronDown className="w-3 h-3 ml-1 rotate-180" /></>
                        ) : (
